...
class TrainTester(BaseTrainTester):
    ...
    def get_model(args):
        ...
        model = BeaUTyDETR(
            ...
            use_spatial_backbone_adapter=getattr(
                args, 'use_spatial_backbone_adapter', False
            ),
            use_structured_slots=getattr(args, 'use_structured_slots', False),
            slot_pooling=getattr(args, 'slot_pooling', 'attention'),
            max_rel_anchor_pairs=getattr(args, 'max_rel_anchor_pairs', 3),
            structured_debug=getattr(args, 'structured_debug', False),
            ...
            sacr_disable_relation=getattr(args, 'sacr_disable_relation', False),
            use_rapf=getattr(args, 'use_rapf', False),
            rapf_hidden_dim=getattr(args, 'rapf_hidden_dim', 128),
            rapf_initial_gate_bias=getattr(args, 'rapf_initial_gate_bias', -2.0),
            rapf_use_quality=getattr(args, 'rapf_use_quality', False),
            rapf_quality_weight=getattr(args, 'rapf_quality_weight', 0.25),
            rapf_struct_residual_clip=getattr(
                args, 'rapf_struct_residual_clip', 2.0
            ),
            rapf_generic_gate_cap=getattr(args, 'rapf_generic_gate_cap', 0.35),
            rapf_quality_anchor_structured_residual=getattr(
                args, 'rapf_quality_anchor_structured_residual', False
            ),
            use_qahnl=getattr(args, 'use_qahnl', False),
            qahnl_score_source=getattr(args, 'qahnl_score_source', 'fused'),
            aux_scores_use_contrastive_base=getattr(
                args, 'aux_scores_use_contrastive_base', False
            ),
            aux_scores_use_semantic_eval_base=getattr(
                args, 'aux_scores_use_semantic_eval_base', False
            ),
            use_semantic_rerank_head=getattr(
                args, 'use_semantic_rerank_head', False
            ),
            semantic_rerank_hidden_dim=getattr(
                args, 'semantic_rerank_hidden_dim', 128
            ),
            semantic_rerank_residual_scale=getattr(
                args, 'semantic_rerank_residual_scale', 0.1
            ),
            semantic_rerank_use_target_conditioning=getattr(
                args, 'semantic_rerank_use_target_conditioning', False
            ),
            use_semantic_rerank_aux_head=bool(getattr(
                args, 'semantic_rerank_aux_checkpoint', ''
            )),
            semantic_rerank_aux_weight=getattr(
                args, 'semantic_rerank_aux_weight', 0.5
            ),
            use_semantic_threshold_head=getattr(
                args, 'use_semantic_threshold_head', False
            ),
            semantic_threshold_hidden_dim=getattr(
                args, 'semantic_threshold_hidden_dim', 64
            ),
            semantic_threshold_residual_scale=getattr(
                args, 'semantic_threshold_residual_scale', 0.25
            ),
            use_semantic_component_calibration=getattr(
                args, 'use_semantic_component_calibration', False
            ),
            semantic_component_max_delta=getattr(
                args, 'semantic_component_max_delta', 0.25
            ),
            semantic_component_use_eda_score=getattr(
                args, 'semantic_component_use_eda_score', False
            ),
            semantic_component_extra_max_weight=getattr(
                args, 'semantic_component_extra_max_weight', 0.25
            ),
            use_semantic_support_adapter=getattr(
                args, 'use_semantic_support_adapter', False
            ),
            semantic_support_overlap_weight=getattr(
                args, 'semantic_support_overlap_weight', 0.6075
            ),
            semantic_support_position_weight=getattr(
                args, 'semantic_support_position_weight', 0.1075
            ),
            semantic_support_overlap_power=getattr(
                args, 'semantic_support_overlap_power', 0.5
            ),
            semantic_support_use_learned_gate=getattr(
                args, 'semantic_support_use_learned_gate', False
            ),
            semantic_support_gate_hidden_dim=getattr(
                args, 'semantic_support_gate_hidden_dim', 16
            ),
            semantic_support_gate_max=getattr(
                args, 'semantic_support_gate_max', 2.0
            ),
            semantic_support_gate_use_query_features=getattr(
                args, 'semantic_support_gate_use_query_features', False
            ),
        )
        return model
    ...
    def _get_inputs(batch_data):
        return {
            **{
            ...
            "det_logits": batch_data.get('all_detected_logits', None),
        }
