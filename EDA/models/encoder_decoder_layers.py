...
def calc_pairwise_locs(obj_centers, eps=1e-10):
    """Build five relative-geometry channels for spatial attention."""
    pairwise = obj_centers.unsqueeze(2) - obj_centers.unsqueeze(1)
    pairwise_dist = torch.sqrt((pairwise ** 2).sum(dim=-1) + eps)
    pairwise_dist_2d = torch.sqrt(
        (pairwise[..., :2] ** 2).sum(dim=-1) + eps
    )
    return torch.stack(
        [
            pairwise_dist,
            pairwise[..., 2] / pairwise_dist,
            pairwise_dist_2d / pairwise_dist,
            pairwise[..., 1] / pairwise_dist_2d,
            pairwise[..., 0] / pairwise_dist_2d,
        ],
        dim=-1,
    )


class CheckpointCompatibleSpatialAttention(nn.Module):
    """Spatial self-attention adapter for generic backbone checkpoints."""

    def __init__(self, d_model, n_head, dropout=0.1, spatial_dim=5):
        super().__init__()
        if d_model % n_head != 0:
        ...
        self.lang_cond_fc = nn.Linear(
            d_model, n_head * (spatial_dim + 1)
        )

    def _split_heads(self, tensor):
        batch_size, sequence_length, _ = tensor.shape
        return tensor.reshape(
            batch_size, sequence_length, self.n_head, self.d_per_head
        ).permute(2, 0, 1, 3)

    def forward(self, q, k, v, pairwise_locs, key_padding_mask=None,
                txt_embeds=None):
        if pairwise_locs is None or txt_embeds is None:
            raise ValueError(
                "spatial attention requires pairwise locations and text"
            )
        residual = q
        q_heads = self._split_heads(self.w_qs(q))
        k_heads = self._split_heads(self.w_ks(k))
        v_heads = self._split_heads(self.w_vs(v))
        attention = torch.einsum(
            'hblk,hbtk->hblt', q_heads, k_heads
        ) / (self.d_per_head ** 0.5)

        spatial_weights = self.lang_cond_fc(
            residual + txt_embeds.unsqueeze(1)
        ).reshape(
            residual.shape[0], residual.shape[1],
            self.n_head, self.spatial_dim + 1,
        ).permute(2, 0, 1, 3)
        location_attention = torch.sigmoid(
            torch.einsum(
                'hbld,bltd->hblt',
                spatial_weights[..., 1:], pairwise_locs,
            ) + spatial_weights[..., :1]
        )

        if key_padding_mask is not None:
            mask = key_padding_mask.unsqueeze(0).unsqueeze(2).expand(
                self.n_head, -1, q.shape[1], -1
            )
            attention = attention.masked_fill(mask, float('-inf'))
            location_attention = location_attention.masked_fill(mask, 0)

        fused_attention = F.softmax(
            attention + torch.log(location_attention.clamp(min=1e-6)),
            dim=-1,
        )
        output = torch.einsum(
            'hblt,hbtv->hblv', fused_attention, v_heads
        ).permute(1, 2, 0, 3).reshape(q.shape)
        output = self.dropout(self.fc(output))
        output = self.layer_norm(output + residual)
        return output, fused_attention
...
class BiEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model=256, dropout=0.1, activation="relu", n_heads=8,
        ...
        if self_attend_vis and self.use_spatial_backbone_adapter:
            self.self_attention_visual = CheckpointCompatibleSpatialAttention(
                d_model=d_model,
                n_head=n_heads,
                dropout=dropout,
            )
        elif self_attend_vis:
    ...
    def forward(self, vis_feats, pos_feats, padding_mask, text_feats,
        ...
        if self.self_attention_visual is not None:
            if self.use_spatial_backbone_adapter:
                global_text = torch.max(text_feats, dim=1)[0]
...
class BiDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_heads, dim_feedforward=2048, dropout=0.1,
        ...
        if self.use_spatial_backbone_adapter:
            self.self_attn = CheckpointCompatibleSpatialAttention(
                d_model=d_model,
                n_head=n_heads,
                dropout=dropout,
            )
    ...
    def forward(self, query, vis_feats, lang_feats, query_pos,
        ...
        if self.self_posembed is not None:
            if self.use_spatial_backbone_adapter:
                spatial_point_xyz = calc_pairwise_locs(query_pos[:, :, :3])
        ...
        # step 1. self-attention
        if self.use_spatial_backbone_adapter:
            global_text = torch.max(lang_feats, dim=1)[0]
