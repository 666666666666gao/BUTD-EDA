...
def load_summary():
    ...
    for job_id in EXPECTED:
        ...
        require(matched[0]["status"] == "completed", "{} is not completed".format(job_id))
        result[job_id] = matched[0]
    require(not [r for r in rows if r.get("status") != "completed"], "summary contains failed/non-completed rows")
    return result
...
def audit_job(job_id, expected, summary):
    ...
    require(len(validations) == 20, "{} has {} validation logs, expected 20".format(job_id, len(validations)))
    epochs = sorted(epoch for epoch, _ in validations)
    require(epochs == list(range(5, 101, 5)), "{} validation epochs are incomplete".format(job_id))
...
def write_outputs(rows):
    ...
    lines = [
        ...
        "Overall status: **PASS**",
        "",
        "All seven rows were independently retrained for epochs 1-100 from the same verified official initialization; no row used `checkpoint_path`. Validation ran every 5 epochs, only the strict maximum official BBS Acc@0.25 checkpoint was retained per row, its SHA256 was verified, no additional weight file remained, and final evaluation reloaded that checkpoint.",
        "",
    ...
    lines.extend(["", "Unique/Multiple counts for every final evaluation: 1,419 + 8,089 = 9,508.", ""])
...
def audit():
    ...
    require(len({row["run_dir"] for row in rows}) == 7, "run directories are not independent")
    write_outputs(rows)
