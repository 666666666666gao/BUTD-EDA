...
def main():
    ...
    require(len(rows) == 10, "expected ten audited rows")
    require({row["job_id"] for row in rows} == EXPECTED, "audited job set differs")
    require(len({row["run_dir"] for row in rows}) == 10, "run directories are not independent")
    for audit in (a1, a2):
        protocol = audit["protocol"]
        require(protocol["dataset"] == "ScanRefer only", "dataset scope differs")
        require(protocol["epochs"] == "1-100", "epoch protocol differs")
    ...
    payload = {
        ...
        "scope": "10 dependency-aware ScanRefer ablations",
        "rows": rows,
