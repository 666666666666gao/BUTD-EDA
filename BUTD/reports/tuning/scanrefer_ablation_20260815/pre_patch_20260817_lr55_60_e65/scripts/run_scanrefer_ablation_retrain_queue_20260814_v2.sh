...
export NMV2_EARLY_STOP_MIN_EPOCH="${NMV2_EARLY_STOP_MIN_EPOCH:-35}"
export NMV2_EARLY_STOP_PATIENCE="${NMV2_EARLY_STOP_PATIENCE:-4}"
export NMV2_EARLY_STOP_MIN_DELTA="${NMV2_EARLY_STOP_MIN_DELTA:-0.001}"
...
PAPER_BASELINE="${QUEUE_ROOT}/butd_paper_baseline.json"
...
register_paper_baseline() {
  /root/miniconda3/envs/bdetr/bin/python - "${PAPER_BASELINE}" "${SUMMARY_TSV}" <<'PY'
import csv, hashlib, json, os, sys
from pathlib import Path

paper_path, summary_path = map(Path, sys.argv[1:])
payload = {
    "job_id": "01_baseline",
    "source_type": "paper",
    "method": "BUTD-DETR",
    "citation": "Jain et al., Bottom Up Top Down Detection Transformers for Language Grounding in Images and Point Clouds, ECCV 2022",
    "source_url": "https://arxiv.org/abs/2112.08879",
    "source_version": "arXiv:2112.08879v5",
    "source_tables": ["Table 1", "Supplementary Table 8"],
    "dataset": "ScanRefer validation",
    "evaluation": "detected boxes, top-1",
    "protocol_note": "External paper reference; the original BUTD-DETR paper used ground-truth text labels. Ablation variants use the repository ScanRefer spaCy parsing protocol and are independently retrained.",
    "metrics": {
        "unique_acc025": 0.842,
        "unique_acc050": 0.663,
...
tmp = paper_path.with_name(paper_path.name + ".tmp.{}".format(os.getpid()))
tmp.write_text(text, encoding="utf-8")
os.replace(str(tmp), str(paper_path))
digest = hashlib.sha256(text.encode("utf-8")).hexdigest()
(paper_path.with_suffix(paper_path.suffix + ".sha256")).write_text(
    "{}  {}\n".format(digest, paper_path), encoding="utf-8"
)

with summary_path.open(newline="") as handle:
    rows = list(csv.DictReader(handle, delimiter="\t"))
fields = ["job_id", "status", "start_time", "end_time", "run_dir", "metric", "best_checkpoint", "sha256"]
rows = [row for row in rows if row.get("job_id") != "01_baseline"]
paper_row = {
    "job_id": "01_baseline",
    "status": "external_paper",
    "start_time": "not_applicable",
    "end_time": "not_applicable",
    "run_dir": str(paper_path),
    "metric": "paper_overall_acc0.25=0.522",
    "best_checkpoint": "",
    "sha256": digest,
}
tmp = summary_path.with_name(summary_path.name + ".tmp.{}".format(os.getpid()))
...
print("REGISTERED_EXTERNAL_PAPER_BASELINE {}".format(digest))
PY
  rm -f "${STATUS_DIR}/01_baseline.failed"
  touch "${STATUS_DIR}/01_baseline.done"
}
...
validate_dry_run() {
  ...
  grep -q -- '--early_stopping' "${dry_log}"
  grep -q -- '--early_stopping_metric last__bbs_acc0.25_top1' "${dry_log}"
  grep -q -- '--early_stopping_min_epoch 35' "${dry_log}"
  grep -q -- '--early_stopping_patience 4' "${dry_log}"
  grep -q -- '--early_stopping_min_delta 0.001' "${dry_log}"
}
...
early_stop = receipt.parent / 'early_stopping.json'
if not config.is_file() or not final_eval.is_file() or not early_stop.is_file():
    raise SystemExit('missing config, early-stop receipt, or final best-model evaluation')
...
register_paper_baseline
sha256sum main_utils.py src/grounding_evaluator.py models/bdetr.py models/losses.py models/reliability_fusion.py scripts/ablations/scanrefer_20260814/*.sh scripts/run_scanrefer_ablation_retrain_queue_20260814_v2.sh tests/test_scanrefer_ablation_launchers.py tests/test_bbs_subset_metrics.py tests/test_best_checkpoint_final_reload.py tests/test_optional_module_rng_isolation.py tests/test_validation_early_stopping.py tools/best_checkpoint_reload_smoke.py tools/model_init_parity_scanrefer.py tools/bridge_live_scanrefer_early_stop.py tools/audit_scanrefer_ablation_completion.py scripts/scanrefer_ablation_watchdog_20260814.sh > "${QUEUE_ROOT}/code_and_launchers.sha256"
