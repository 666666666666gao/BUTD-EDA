...
def rewrite_summary(best, checkpoint, digest):
    fields = [
    ...
    rows.append({
        ...
        "end_time": now(),
        "run_dir": str(RUN_DIR),
        "metric": "{}={}".format(PRIMARY, best["score"]),
...
def bridge_stop(stopper, event, rows):
    ...
    # Wait until the legacy queue has recorded its expected non-zero exit, then
    # replace that transient state with the independently verified completion.
    archive_failed_marker()
    rewrite_summary(best, checkpoint, digest)
    atomic_text(STATUS / (JOB_ID + ".done"), now() + "\n")
    atomic_text(
        BRIDGE_COMPLETE,
        json.dumps({
            "job_id": JOB_ID,
            "run_dir": str(RUN_DIR),
            "stop_epoch": int(event["epoch"]),
    ...
    resume_services()
    print(
        "{} EXTERNAL_EARLY_STOP_BRIDGE_PASS stop_epoch={} best_epoch={} "
        "best_score={:.10f}".format(
