...
def audit_job(job_id, expected, summary):
    ...
    require(early_path.is_file(), "missing early-stopping receipt")
    ...
    require(early_policy["status"] in ("early_stopped", "max_epoch"), "invalid early-stop status")
    ...
    require(35 <= stop_epoch <= 100 and stop_epoch % 5 == 0, "invalid stop epoch")
    if early["status"] == "early_stopped":
        require(stop_epoch < 100, "early stop cannot equal maximum epoch")
        require(int(early.get("stale_validations", -1)) >= 4, "stopped before patience")
        require(early.get("reason") == "validation_metric_saturated", "wrong stop reason")
    else:
        require(stop_epoch == 100, "max-epoch receipt ended early")
    epochs = sorted(epoch for epoch, _ in validations)
    require(epochs == list(range(5, stop_epoch + 1, 5)), "validation epochs do not match stop receipt")
...
def main():
    ...
    lines = [
        ...
        "All three extension rows independently retrained from epoch 1 with a 100-epoch ceiling and the locked validation-based early-stopping rule. Each retains exactly one strict-best official Overall Acc@0.25 checkpoint and passed final best-checkpoint reload evaluation.", "",
