...
PAPER_BASELINE = QUEUE1 / "butd_paper_baseline.json"

ROWS = [
    ("module", "01_baseline", "BUTD-DETR (paper)", 0, 0, 0, 0, 0, 0),
...
def current_result(job_id):
    if job_id == "01_baseline" and PAPER_BASELINE.is_file():
        ...
        return {
            ...
            "source_type": "paper",
            "best_epoch": "paper",
            "unique025": metrics["unique_acc025"],
            "unique050": metrics["unique_acc050"],
            "multiple025": metrics["multiple_acc025"],
            "multiple050": metrics["multiple_acc050"],
            "overall025": metrics["overall_acc025"],
            "overall050": metrics["overall_acc050"],
        }
    receipts = sorted((TRAIN / job_id / "scanrefer_spacy").glob("*/best_primary.json"), key=lambda p: p.stat().st_mtime)
    if not receipts:
        return {"status": "queued", "source_type": "trained", "best_epoch": "-"}
    ...
    result = {"status": "completed" if completed(job_id) else "training", "source_type": "trained", "best_epoch": epoch}
    if metrics:
...
def main():
    ...
    fields = [
        "group", "job_id", "setting", "status", "source_type", "best_epoch", "sacr", "rapf", "qahnl",
    ...
    for row in data:
        suffix = "‡" if row["status"] == "external_paper" else ("†" if row["status"] == "training" else "")
    ...
    md.extend([
        "", "The BUTD-DETR baseline is taken from Jain et al. (ECCV 2022), Table 1 and Supplementary Table 8: https://arxiv.org/abs/2112.08879. The original paper used ground-truth text labels. All trained ablation variants use ScanRefer only with the repository spaCy parsing protocol, seed 0, a 100-epoch ceiling, validation every 5 epochs, and one locked early-stopping rule (official Overall Acc@0.25, minimum epoch 35, patience 4 validations, minimum gain 0.10 percentage point), with the same verified official detector initialization. Each completed trained row retains only the strict-best official Overall Acc@0.25 checkpoint.",
    ...
    for index, row in enumerate(data):
        ...
        if row["status"] == "external_paper":
            suffix = r"$^{\ddagger}$"
        elif row["status"] == "training":
            suffix = r"$^{\dagger}$"
        else:
            suffix = ""
        label = row["setting"].replace("_", r"\_") + suffix
