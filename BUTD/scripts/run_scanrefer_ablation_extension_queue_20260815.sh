...
export NMV2_BATCH_SIZE=24 NMV2_MAX_EPOCH=65 NMV2_VAL_FREQ=5 NMV2_PRINT_FREQ=100 NMV2_LR_DECAY_EPOCHS="55 60"
...
sha256sum \
  main_utils.py utils/lr_scheduler.py src/grounding_evaluator.py models/bdetr.py models/losses.py models/reliability_fusion.py \
  ...
  tests/test_scanrefer_ablation_extension_launchers.py tests/test_optional_module_rng_isolation.py tests/test_validation_early_stopping.py tests/test_lr_scheduler_epoch_milestones.py \
...
while IFS=$'\t' read -r job_id script_path; do
  ...
  grep -q -- '--max_epoch 65' "${LAUNCH_LOG_DIR}/${job_id}.dryrun.txt"
  grep -q -- '--lr_decay_epochs 55 60' "${LAUNCH_LOG_DIR}/${job_id}.dryrun.txt"
  grep -q -- '--best_checkpoint_only' "${LAUNCH_LOG_DIR}/${job_id}.dryrun.txt"
  if grep -q -- '--early_stopping' "${LAUNCH_LOG_DIR}/${job_id}.dryrun.txt"; then
    echo "ERROR: ${job_id} unexpectedly enables early stopping" >&2; exit 3
  fi
