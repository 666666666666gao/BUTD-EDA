...
export NMV2_MAX_EPOCH="${NMV2_MAX_EPOCH:-65}"
export NMV2_VAL_FREQ="${NMV2_VAL_FREQ:-5}"
export NMV2_PRINT_FREQ="${NMV2_PRINT_FREQ:-100}"
export NMV2_LR_DECAY_EPOCHS="${NMV2_LR_DECAY_EPOCHS:-55 60}"
...
validate_dry_run() {
  ...
  grep -q -- '--max_epoch 65' "${dry_log}"
  grep -q -- '--lr_decay_epochs 55 60' "${dry_log}"
  if grep -q -- '--early_stopping' "${dry_log}"; then
    echo "ERROR: ${job_id} unexpectedly enables early stopping" >&2
    return 3
  fi
}
...
register_paper_baseline
sha256sum main_utils.py utils/lr_scheduler.py src/grounding_evaluator.py models/bdetr.py models/losses.py models/reliability_fusion.py scripts/ablations/scanrefer_20260814/*.sh scripts/run_scanrefer_ablation_retrain_queue_20260814_v2.sh tests/test_scanrefer_ablation_launchers.py tests/test_bbs_subset_metrics.py tests/test_best_checkpoint_final_reload.py tests/test_optional_module_rng_isolation.py tests/test_validation_early_stopping.py tests/test_lr_scheduler_epoch_milestones.py tools/best_checkpoint_reload_smoke.py tools/model_init_parity_scanrefer.py tools/bridge_live_scanrefer_early_stop.py tools/audit_scanrefer_ablation_completion.py scripts/scanrefer_ablation_watchdog_20260814.sh > "${QUEUE_ROOT}/code_and_launchers.sha256"
...
/root/miniconda3/envs/bdetr/bin/python -m pytest -q \
  ...
  tests/test_lr_scheduler_epoch_milestones.py \
