...
TORCH_DISTRIBUTED_DEBUG=INFO CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" "${DIST_LAUNCH[@]}" \
    ...
    --log_dir "./logs/scanrefer_spacy/onestage/block6_full_method_scanrefer" \
    --use_structured_slots \
    --use_late_acd \
    --use_dhc \
    --slot_pooling attention \
    --max_rel_anchor_pairs 4 \
    --acd_top_m_targets 36 \
    --acd_top_k_anchors 14 \
    --acd_use_confidence_fusion \
    --acd_global_residual_alpha 0.3 \
    --acd_initial_alpha 0.05 \
    --acd_warmup_steps 10000 \
    --acd_ea_scale 0.9 \
    --dhc_consistency_weight 0.08 \
    --dhc_ent_hardneg_weight 0.1 \
    --dhc_attr_hardneg_weight 0.15 \
