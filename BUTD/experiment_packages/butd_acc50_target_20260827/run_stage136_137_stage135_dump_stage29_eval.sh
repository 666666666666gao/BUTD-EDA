...
TRAIN_OUT="${ROOT}/stage135_stage29_option_last_box_noaug"
mapfile -t CKPTS < <(find "$TRAIN_OUT" -type f -name ckpt_best_primary.pth | sort)
test "${#CKPTS[@]}" = 1
CKPT="${CKPTS[0]}"
OUT="${ROOT}/stage136_stage135_raw_val_geometry_dump"
PATCHED="${P}/grounding_evaluator_adapter_dump_parityfixed.py"
LIVE="src/grounding_evaluator.py"
BACKUP="${P}/state/stage136_stage135_raw_val_dump_20260830"
DUMP="${OUT}/stage135_e12_raw_val_geometry.pt"
...
RESULT="${ROOT}/stage137_stage29_on_stage135_locked_val_eval.json"
STATUS="${P}/stage136_137_chain_status.txt"
...
assert_storage
mkdir -p "$BACKUP" "$OUT"
cp -p "$LIVE" "$BACKUP/grounding_evaluator.pre_stage136.py"
chmod 0444 "$BACKUP/grounding_evaluator.pre_stage136.py"
restore() {
  install -m 0644 "$BACKUP/grounding_evaluator.pre_stage136.py" "$LIVE"
}
...
printf 'stage136_dumping %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
NMV2_DETECTOR_TOPK_COMPACT_DUMP_PATH="$DUMP" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
  2>&1 | tee "$P/stage136_stage135_raw_val_dump.log"
...
print('STAGE136_DUMP_PASS rows={} acc025={:.12f} acc050={:.12f} bytes={}'.format(
    len(rows), metrics['last__bbs_acc0.25_top1'],
    metrics['last__bbs_acc0.50_top1'], os.path.getsize(dump)))
PY

printf 'stage137_evaluating %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
"${PYTHON}" "$RANKER_SCRIPT" evaluate \
  "$DUMP" "$MODEL" "$LOCK" "$RESULT" \
  2>&1 | tee "$P/stage137_stage29_on_stage135_locked_val_eval.log"
...
path = os.path.join(out, 'stage136_137_receipt.json')
...
chmod 0444 "$OUT/stage136_137_receipt.json" "$OUT/launch_manifest.txt"
printf 'stage137_complete %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
