...
def _run(scores, iou, candidate_mask=None, threshold50_direct_weight=0.0):
    ...
    result = losses._qahnl_tiered_ordinal_losses(
        ...
        margin10=0.10,
        temperature=1.0,
        threshold50_direct_weight=threshold50_direct_weight,
    )
...
def test_threshold50_direct_constraint_targets_strongest_non_tier2():
    # The ambiguous query has the highest wrong score.  Adjacent-only ordinal
    # relations deliberately ignore it, while the threshold-safe @.50 term
    # must push it below the best Tier-2 query.
    scores, result = _run(
        [[0.0, -1.0, -2.0, 4.0]],
        [[0.70, 0.30, 0.05, 0.20]],
        threshold50_direct_weight=3.0,
    )
    assert result['active2x_count'].item() == 1
    assert result['violation2x'].item() == 1.0
    result['loss_raw'].backward()
    assert scores.grad[0, 0].item() < 0.0
...
if __name__ == '__main__':
    ...
    test_threshold50_direct_constraint_targets_strongest_non_tier2()
