...
export MASTER_PORT="${MASTER_PORT:-33948}"
...
OUT="${ROOT}/stage147a_tier_pair_zero_gate_parity_eval"
LOG="${P}/stage147a_tier_pair_zero_gate_parity_eval.log"
STATUS="${P}/stage147a_tier_pair_zero_gate_parity_status.txt"
...
mkdir -p "$OUT"
{
  printf 'stage=147a_tier_pair_zero_gate_parity\n'
...
print('STAGE147A_EXACT_PARITY_PASS acc025={:.15f} acc050={:.15f}'.format(
    actual25, actual50
))
PY
chmod 0444 "$OUT/launch_manifest.txt" "$OUT/eval_results.json"
printf 'stage147a_complete %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
