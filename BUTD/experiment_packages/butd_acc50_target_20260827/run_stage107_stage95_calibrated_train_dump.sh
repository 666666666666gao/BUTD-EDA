...
export MASTER_PORT="${MASTER_PORT:-33907}"
source "${R}/experiment_packages/scanrefer_monotonic_main_ablations_20260825/common.sh"
cd "${R}"

OUT="${STAGE107_OUT:-${ROOT}/stage107_stage95_e11_calibrated_train_geometry_dump}"
...
BACKUP="${STAGE107_BACKUP:-${P}/state/stage107_stage95_calibrated_train_dump_20260830}"
DUMP="${OUT}/stage95_e11_calibrated_train_geometry.pt"
STATUS="${STAGE107_STATUS:-${P}/stage107_calibrated_train_dump_status.txt}"
EXPECTED_ROWS="${STAGE107_EXPECTED_ROWS:-36665}"
MAX_SAMPLES="${STAGE107_MAX_SAMPLES:--1}"
...
assert_storage
mkdir -p "${BACKUP}" "${OUT}"
cp -p "${LIVE}" "${BACKUP}/grounding_evaluator.pre_stage107.py"
chmod 0444 "${BACKUP}/grounding_evaluator.pre_stage107.py"
restore() {
  install -m 0644 "${BACKUP}/grounding_evaluator.pre_stage107.py" "${LIVE}"
}
...
printf 'stage107_dumping %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
NMV2_DETECTOR_TOPK_COMPACT_DUMP_PATH="${DUMP}" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
  2>&1 | tee "${P}/stage107_stage95_calibrated_train_dump.log"
...
chmod 0444 "${OUT}/stage107_receipt.json" "${OUT}/launch_manifest.txt"
printf 'stage107_complete %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
