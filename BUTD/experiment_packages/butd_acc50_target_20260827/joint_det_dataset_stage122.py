    ...
    def _get_detected_objects(self, split, scan_id, augmentations):
        ...
        if (
            ...
            corrupt_probability = float(
                os.environ.get("BUTD_AUGMENT_DET_CORRUPT_PROB", "0.3")
            )
            if not 0.0 <= corrupt_probability <= 1.0:
                raise ValueError(
                    "BUTD_AUGMENT_DET_CORRUPT_PROB must be in [0, 1], "
                    f"got {corrupt_probability}"
                )
            corrupt = (
                np.random.random(len(valid_bboxes))
