...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage18_geometry_action_head"
...
CMD+=(--log_dir "${OUT}" --max_epoch "$((E+2))" --val_freq 1 --save_freq 1000
 ...
 --detector_policy_adapter_delta_scale 4.0 --detector_policy_adapter_lr 0.0005
...
mkdir -p "${OUT}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" 2>&1 | tee "${P}/stage18_train.log"
