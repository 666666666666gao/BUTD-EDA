...
OUT="${ROOT}/stage151a_stage150_e13_calibrated_tier_dump"
DUMP="${OUT}/stage150_e13_calibrated_geometry.pt"
FIXBREAK="${OUT}/stage151_stage150_fix_break_report.json"
TIER_REPORT="${OUT}/stage151_remaining_tier_report.json"
STATUS="${P}/stage151_calibrated_tier_status.txt"
...
ANALYZE="${P}/analyze_stage151_remaining_tiers.py"
STATE="${P}/state/stage151_stage150_calibrated_tier_20260831"
...
EXPECTED_ANALYZE_SHA="b6b427cd4bed585a51d5a764d7ba7696dd62f2e24a5c7e41738b59ea448a4746"
...
fail_status() {
  local rc=$?
  printf 'stage151_failed rc=%s at=%s line=%s\n' \
...
assert_storage
mkdir -p "${OUT}" "${STATE}"
cp -p "${LIVE}" "${STATE}/grounding_evaluator.pre_stage151.py"
chmod 0444 "${STATE}/grounding_evaluator.pre_stage151.py"
restore() {
  install -m 0644 "${STATE}/grounding_evaluator.pre_stage151.py" "${LIVE}"
}
...
{
  printf 'stage=151_stage150_calibrated_remaining_tier\n'
  printf 'started_at=%s\ncheckpoint=%s\ncheckpoint_sha256=%s\n' \
...
print('STAGE151_STAGE150_CALIBRATED_PARITY_PASS', hits)
PY

"${PYTHON}" "${COMPARE}" "${BASELINE_DUMP}" "${DUMP}" "${FIXBREAK}" \
  > "${P}/stage151_stage150_fix_break_compare.log"
"${PYTHON}" "${ANALYZE}" "${BASELINE_DUMP}" "${DUMP}" "${TIER_REPORT}" \
  > "${P}/stage151_remaining_tier_analysis.log"
...
print('STAGE151_REPORT_PARITY_PASS')
PY
