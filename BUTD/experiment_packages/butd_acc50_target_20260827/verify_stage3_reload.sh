...
CHECKPOINT="${1:?usage: verify_stage3_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
QUALITY_WEIGHT="${2:?usage: verify_stage3_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
CALIBRATION_JSON="${3:?usage: verify_stage3_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
...
VERIFY_ROOT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage3_reload_verify"
...
[ -f "${CHECKPOINT}" ] || {
  echo "ERROR: Stage-3 source checkpoint missing: ${CHECKPOINT}" >&2
  exit 140
}
[ -f "${CALIBRATION_JSON}" ] || {
  echo "ERROR: Stage-3 calibration receipt missing: ${CALIBRATION_JSON}" >&2
  exit 141
}
[ ! -e "${VERIFY_ROOT}" ] || {
...
"${PYTHON}" - "${RESULT_JSON}" "${CHECKPOINT}" "${RECEIPT}" \
  "${QUALITY_WEIGHT}" "${CALIBRATION_JSON}" <<'PY'
import hashlib
import json
import os
import sys

result_path, checkpoint_path, receipt_path, weight, calibration_path = sys.argv[1:]
