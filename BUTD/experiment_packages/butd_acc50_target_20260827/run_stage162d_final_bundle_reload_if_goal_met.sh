...
STATUS="${P}/stage162d_dependency_status.txt"
UPSTREAM_STATUS="${P}/stage162b_dependency_status.txt"
...
SELECTOR_LOCK="${ROOT}/stage162a_stage135c_tier3_option_ranker/locked_tier3_option_policy.json"
LOCKED_RESULT="${ROOT}/stage162b_tier3_option_ranker_validation_result.json"
BUNDLE="${ROOT}/stage162d_final_artifact_bundle"
TEMP_ALT_CKPT="${ROOT}/stage162d_materialized_stage135c_tmp.pth"
ALT_OUT="${ROOT}/stage162d_fresh_bundle_stage135c_raw_val"
ALT_DUMP="${ALT_OUT}/stage135c_fresh_raw_val_geometry.pt"
FRESH_RESULT="${ROOT}/stage162d_fresh_bundle_option_validation_result.json"
RELOAD_RECEIPT="${ROOT}/stage162d_fresh_bundle_reload_receipt.json"
...
fail_status() {
  ...
  set +e
  cleanup
  printf 'stage162d_failed rc=%s at=%s line=%s\n' \
...
printf 'stage162d_waiting_for_stage162b %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
while ! grep -q -E '^stage162b_(complete_|not_authorized|failed)' \
  "${UPSTREAM_STATUS}" 2>/dev/null; do
  sleep 60
done

if ! grep -q '^stage162b_complete_goal_met ' "${UPSTREAM_STATUS}"; then
  printf 'stage162d_not_authorized_offline_goal_not_met %s\n' \
    "$(date --iso-8601=seconds)" > "${STATUS}"
  chmod 0444 "${STATUS}"
  trap - ERR INT TERM
  exit 0
...
printf 'stage162d_building_bundle %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
...
assert_storage
mkdir -p "${ALT_OUT}"
PRE_EVAL="${ALT_OUT}/grounding_evaluator.pre_stage162d_alt.py"
cp -p "${LIVE}" "${PRE_EVAL}"
...
print('STAGE162D_ALTERNATE_FRESH_PARITY_PASS', hits)
PY
...
printf 'stage162d_fresh_locked_option_ranker %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" "${BUNDLE}/runtime/stage162_tier3_option_ranker.py" evaluate \
  "${ALT_DUMP}" "${BUNDLE}/locks/stage153.json" "${FRESH_RESULT}" \
  2>&1 | tee "${P}/stage162d_fresh_locked_option_ranker.log"
...
def sha(path):
    ...
    return h.hexdigest()
receipt={
  'stage':'stage162d_fresh_bundle_reload',
  'status':'complete',
...
"${PYTHON}" "${BUNDLE}/runtime/stage153_finalize_artifact.py" \
  ...
  2>&1 | tee "${P}/stage162d_finalize_bundle.log"
"${PYTHON}" "${BUNDLE}/runtime/stage153_build_final_artifact.py" validate \
  "${BUNDLE}" 2>&1 | tee "${P}/stage162d_final_bundle_validate.log"

cleanup
trap - ERR INT TERM
printf 'stage162d_complete_fresh_bundle_reload %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
