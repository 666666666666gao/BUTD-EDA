...
TRAIN_OUT="${ROOT}/stage135b_stage29_option_last_box_noaug_exampleid"
mapfile -t CKPTS < <(find "$TRAIN_OUT" -type f -name ckpt_best_primary.pth | sort)
test "${#CKPTS[@]}" = 1
CKPT="${CKPTS[0]}"
OUT="${ROOT}/stage136b_stage135b_raw_val_geometry_dump"
PATCHED="${P}/grounding_evaluator_adapter_dump_parityfixed.py"
LIVE="src/grounding_evaluator.py"
BACKUP="${P}/state/stage136b_stage135b_raw_val_dump_20260830"
DUMP="${OUT}/stage135b_e12_raw_val_geometry.pt"
...
RESULT="${ROOT}/stage137b_stage29_on_stage135b_locked_val_eval.json"
STATUS="${P}/stage136b_137b_chain_status.txt"
...
assert_storage
mkdir -p "$BACKUP" "$OUT"
cp -p "$LIVE" "$BACKUP/grounding_evaluator.pre_stage136b.py"
chmod 0444 "$BACKUP/grounding_evaluator.pre_stage136b.py"
restore() {
  install -m 0644 "$BACKUP/grounding_evaluator.pre_stage136b.py" "$LIVE"
}
...
printf 'stage136b_dumping %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
NMV2_DETECTOR_TOPK_COMPACT_DUMP_PATH="$DUMP" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
  2>&1 | tee "$P/stage136b_stage135b_raw_val_dump.log"
...
print('STAGE136B_DUMP_PASS rows={} acc025={:.12f} acc050={:.12f} bytes={}'.format(
    len(rows), metrics['last__bbs_acc0.25_top1'],
    metrics['last__bbs_acc0.50_top1'], os.path.getsize(dump)))
PY

printf 'stage137b_evaluating %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
"${PYTHON}" "$RANKER_SCRIPT" evaluate \
  "$DUMP" "$MODEL" "$LOCK" "$RESULT" \
  2>&1 | tee "$P/stage137b_stage29_on_stage135b_locked_val_eval.log"
...
receipt = {
    'stage': '136b_137b', 'status': 'complete',
...
path = os.path.join(out, 'stage136b_137b_receipt.json')
...
chmod 0444 "$OUT/stage136b_137b_receipt.json" "$OUT/launch_manifest.txt"
printf 'stage137b_complete %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
