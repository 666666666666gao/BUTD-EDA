...
OUT="$BASE/stage53_explicit_top2_rescue_selector"
RESULT="$BASE/stage54_explicit_top2_rescue_locked_val_eval.json"
...
SCRIPT="$PKG/train_top2_candidate_rescue_selector.py"
...
sha256sum "$OUT/top2_rescue_selector.txt" \
  "$OUT/locked_top2_rescue_policy.json" > "$OUT/lock_sha256_before_val.txt"
"$PY" "$SCRIPT" evaluate "$VAL_DUMP" \
  "$OUT/top2_rescue_selector.txt" "$OUT/locked_top2_rescue_policy.json" \
  "$RESULT" \
  --top1-script "$TOP1_SCRIPT" --top2-script "$TOP2_SCRIPT" \
  --top1-model "$TOP1_MODEL" --top2-model "$TOP2_MODEL" \
  --top1-lock "$TOP1_LOCK" --top2-lock "$TOP2_LOCK"
sha256sum "$OUT/top2_rescue_selector.txt" \
  "$OUT/locked_top2_rescue_policy.json" > "$OUT/lock_sha256_after_val.txt"
