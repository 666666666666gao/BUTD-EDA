...
STATUS="${P}/stage165d_dependency_status.txt"
UPSTREAM_STATUS="${P}/stage165b_dependency_status.txt"
...
STAGE31_LOCK="${ROOT}/stage164a_same_domain_retrained_trio/inner_blend/locked_blend_policy.json"
STAGE33_LOCK="${ROOT}/stage164a_same_domain_retrained_trio/pointwise/locked_pointwise_policy.json"
NESTED_LOCK="${ROOT}/stage165a_capped_same_domain_trio/locked_capped_nested_policy.json"
AUTH_LOCK="${ROOT}/stage165a_capped_same_domain_trio/locked_capped_authorization.json"
LOCKED_RESULT="${ROOT}/stage165b_capped_validation_result.json"
BUNDLE="${ROOT}/stage165d_final_artifact_bundle"
TEMP_ALT_CKPT="${ROOT}/stage165d_materialized_stage135c_tmp.pth"
ALT_OUT="${ROOT}/stage165d_fresh_bundle_stage135c_raw_val"
ALT_DUMP="${ALT_OUT}/stage135c_fresh_raw_val_geometry.pt"
RAW_FRESH_RESULT="${ROOT}/stage165d_fresh_raw_nested_validation_result.json"
FRESH_RESULT="${ROOT}/stage165d_fresh_bundle_capped_validation_result.json"
RELOAD_RECEIPT="${ROOT}/stage165d_fresh_bundle_reload_receipt.json"
...
fail_status() {
  ...
  set +e
  cleanup
  printf 'stage165d_failed rc=%s at=%s line=%s\n' \
...
printf 'stage165d_waiting_for_stage165b %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
while ! grep -q -E '^stage165b_(complete_|not_authorized|failed)' \
  "${UPSTREAM_STATUS}" 2>/dev/null; do
  sleep 60
done

if ! grep -q '^stage165b_complete_goal_met ' "${UPSTREAM_STATUS}"; then
  printf 'stage165d_not_authorized_offline_goal_not_met %s\n' \
    "$(date --iso-8601=seconds)" > "${STATUS}"
  chmod 0444 "${STATUS}"
  trap - ERR INT TERM
  exit 0
...
test ! -e "${ALT_OUT}"
test ! -e "${RAW_FRESH_RESULT}"
test ! -e "${FRESH_RESULT}"
test ! -e "${RELOAD_RECEIPT}"

printf 'stage165d_building_bundle %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" "${P}/stage153_build_final_artifact.py" build \
  "${ALT_SOURCE_CKPT}" "${PRIMARY_SOURCE_CKPT}" \
  "${STAGE31_LOCK}" "${STAGE33_LOCK}" "${NESTED_LOCK}" \
  "${AUTH_LOCK}" "${LOCKED_RESULT}" "${BUNDLE}" \
...
assert_storage
mkdir -p "${ALT_OUT}"
PRE_EVAL="${ALT_OUT}/grounding_evaluator.pre_stage165d_alt.py"
cp -p "${LIVE}" "${PRE_EVAL}"
...
print('STAGE165D_ALTERNATE_FRESH_PARITY_PASS', hits)
PY
...
printf 'stage165d_fresh_locked_capped_nested_policy %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" "${BUNDLE}/runtime/stage140_train_eval_nested_blend.py" evaluate \
  "${ALT_DUMP}" "${BUNDLE}/locks/stage142.json" "${RAW_FRESH_RESULT}" \
  2>&1 | tee "${P}/stage165d_fresh_locked_capped_policy.log"
"${PYTHON}" "${BUNDLE}/runtime/stage165_capped_nested_gate.py" \
  wrap-validation "${BUNDLE}/locks/stage153.json" \
  "${RAW_FRESH_RESULT}" "${FRESH_RESULT}" \
  2>&1 | tee -a "${P}/stage165d_fresh_locked_capped_policy.log"
...
def sha(path):
    ...
    return h.hexdigest()
receipt={
  'stage':'stage165d_fresh_bundle_reload',
  'status':'complete',
...
"${PYTHON}" "${BUNDLE}/runtime/stage153_finalize_artifact.py" \
  ...
  2>&1 | tee "${P}/stage165d_finalize_bundle.log"
"${PYTHON}" "${BUNDLE}/runtime/stage153_build_final_artifact.py" validate \
  "${BUNDLE}" 2>&1 | tee "${P}/stage165d_final_bundle_validate.log"

cleanup
trap - ERR INT TERM
printf 'stage165d_complete_fresh_bundle_reload %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
chmod 0444 "${STATUS}" "${RELOAD_RECEIPT}" \
  "${RAW_FRESH_RESULT}" "${FRESH_RESULT}"
