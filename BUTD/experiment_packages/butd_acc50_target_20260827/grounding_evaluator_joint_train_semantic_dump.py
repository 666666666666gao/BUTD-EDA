...
class GroundingEvaluator:
    ...
    def _record_detector_topk_compact(
        ...
        # Training-dump provenance only.  These identifiers are used to split
        # the offline selector by scene so descriptions of the same ScanNet
        # scene cannot occur on both sides of its internal validation split.
        # They are not candidate features and are never consumed at inference.
        for endpoint_key in ('scene_id', 'ann_id', 'object_id'):
            endpoint_value = end_points.get(endpoint_key, None)
            if isinstance(endpoint_value, (list, tuple)):
                if bid < len(endpoint_value):
                    row[endpoint_key] = str(endpoint_value[bid])
            elif torch.is_tensor(endpoint_value):
                if bid < endpoint_value.shape[0]:
                    row[endpoint_key] = str(
                        endpoint_value[bid].detach().cpu().item()
                    )
        ...
        if (
            ...
            # Store candidate-aligned decoder representations compactly.  A
            # single FP16 byte blob per field avoids the large pickle/storage
            # overhead of serializing one tensor (or hundreds of Python
            # floats) per candidate.  The corresponding shape makes decoding
            # explicit and guards candidate-order parity offline.
            for endpoint_key, row_key in (
                ('last_proj_queries', 'adapter_last_proj_query_f16'),
                ('last_queries', 'adapter_last_query_f16'),
            ):
                values = end_points.get(endpoint_key, None)
                if (
                    torch.is_tensor(values)
                    and values.dim() == 3
                    and bid < values.shape[0]
                    and candidate.numel() > 0
                    and values.shape[1] > int(candidate.max().item())
                ):
                    packed = (
                        values[bid][candidate]
                        .detach().to(device='cpu', dtype=torch.float16)
                        .contiguous()
                    )
                    row[row_key] = packed.numpy().tobytes(order='C')
                    row[row_key + '_shape'] = [
                        int(size) for size in packed.shape
                    ]
            for endpoint_key, row_key in (
                (
