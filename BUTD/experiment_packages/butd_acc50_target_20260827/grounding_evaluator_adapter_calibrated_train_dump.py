...
class GroundingEvaluator:
    ...
    def _record_detector_topk_compact(
        ...
        if 'pred_iou' not in end_points:
            return
        calibrated_boxes = end_points.get(
            'detector_policy_adapter_calibrated_boxes', None
        )
        if (
            torch.is_tensor(calibrated_boxes)
            and bid < calibrated_boxes.shape[0]
            and calibrated_boxes[bid].shape == pred_bbox.shape
        ):
            pred_bbox = calibrated_boxes[bid].to(
        ...
        # Training-dump provenance only. These identifiers split the offline
        # selector by scene and are never consumed as inference features.
