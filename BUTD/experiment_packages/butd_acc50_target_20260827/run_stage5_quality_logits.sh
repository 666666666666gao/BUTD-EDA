...
JOB_ROOT="${ROOT}/stage5_quality_head_logits_top5"
...
assert_storage
[ ! -e "${JOB_ROOT}" ] || {
  echo "ERROR: refusing to overwrite existing Stage-5 root ${JOB_ROOT}" >&2
  exit 172
}
mkdir -p "${JOB_ROOT}"
...
[ -f "${CHECKPOINT}" ] || {
  echo "ERROR: Stage-5 best checkpoint missing: ${CHECKPOINT}" >&2
  exit 173
}
{
...
"${PACKAGE_ROOT}/run_stage5_quality_grid.sh" "${CHECKPOINT}"
