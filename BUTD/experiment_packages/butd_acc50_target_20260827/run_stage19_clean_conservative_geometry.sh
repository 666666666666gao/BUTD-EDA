...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage19_clean_conservative_geometry"
...
CMD+=(--log_dir "${OUT}" --max_epoch "$((E+1))" --val_freq 1 --save_freq 1000
 ...
 --detector_policy_adapter_delta_scale 4.0 --detector_policy_adapter_lr 0.0001
...
mkdir -p "${OUT}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" 2>&1 | tee "${P}/stage19_train.log"
