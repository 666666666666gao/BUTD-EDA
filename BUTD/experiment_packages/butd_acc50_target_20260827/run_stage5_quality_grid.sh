...
CHECKPOINT="${1:?usage: run_stage5_quality_grid.sh CHECKPOINT}"
...
[ -f "${CHECKPOINT}" ] || {
  echo "ERROR: Stage-5 checkpoint missing: ${CHECKPOINT}" >&2
  exit 174
}
[ ! -e "${OUT_ROOT}" ] || {
  echo "ERROR: refusing to overwrite Stage-5 calibration root ${OUT_ROOT}" >&2
  exit 175
}
[ ! -e "${VERIFY_ROOT}" ] || {
...
if [ "${SELECTED_FEASIBLE}" = "true" ]; then
  "${PACKAGE_ROOT}/verify_stage5_reload.sh" \
...
payload = {
    ...
    'reason': 'Stage-5 logit-ranking training produced no feasible grid weight',
}
