...
PARITY_OUT='/root/autodl-tmp/logs/butd_acc50_target_20260827/stage58_rank2_rescue_parity'
PARITY_JSON="${PARITY_OUT}/eval_results.json"
PARITY_RECEIPT="${PARITY_OUT}/parity_receipt.json"
OUT='/root/autodl-tmp/logs/butd_acc50_target_20260827/stage58_rank2_rescue_head'
...
append_training_flags() {
  CMD+=(
    --log_dir "${OUT}" --max_epoch "$((E+4))" --val_freq 1
...
# Full validation parity is a hard launch gate.  The fresh rank-2 head starts
# below its zero override threshold and must reproduce Stage18 exactly.
if [ -f "${PARITY_RECEIPT}" ]; then
  "${PYTHON}" - "${PARITY_RECEIPT}" <<'PY'
import json, sys
receipt = json.load(open(sys.argv[1]))
assert receipt['parity_pass'] is True, receipt
PY
else
  [ ! -e "${PARITY_OUT}" ] || {
  ...
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
    2>&1 | tee "${P}/stage58_parity_eval.log"
  "${PYTHON}" - "${PARITY_JSON}" "${PARITY_RECEIPT}" "${SRC}" <<'PY'
...
if not payload['parity_pass']:
    raise SystemExit('rank-2 rescue checkpoint parity failed')
PY
fi
