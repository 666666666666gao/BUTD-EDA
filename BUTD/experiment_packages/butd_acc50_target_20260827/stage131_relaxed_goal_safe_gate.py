...
# This budget is locked from the user-visible strict ScanRefer goal before
# inspecting validation predictions.  Stage95 scores 0.547749 at Acc@0.25 and
# the required floor is 0.5391, leaving 0.008649; reserve a small safety margin
# and allow at most a 0.0085 relative drop on the train-only dev split.
MAX_RELATIVE_ACC025_DROP = 0.0085
...
def choose_threshold(scores, iou18, iou95):
    ...
    feasible = [
        row for row in rows
        if row["acc025"] >= baseline["acc025"] - MAX_RELATIVE_ACC025_DROP
    ]
    return max(
        feasible,
        key=lambda row: (
...
lock = {
    "stage": "131",
    "protocol": "train_fit_dev_goal_budget_locked_cross_checkpoint_safe_gate",
    "max_relative_acc025_drop": MAX_RELATIVE_ACC025_DROP,
    "inputs": {
