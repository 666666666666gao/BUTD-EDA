...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage10_dualthreshold_reranker"
...
CMD+=(--log_dir "${OUT}" --max_epoch "$((E+2))" --val_freq 1 --save_freq 1000
 ...
 --detector_policy_adapter_lr 0.001 --detector_policy_adapter_loss_weight 1.0
...
[ ! -e "${OUT}" ] || { echo "refusing to overwrite ${OUT}" >&2; exit 202; }
mkdir -p "${OUT}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" 2>&1 | tee "${P}/stage10_train.log"
