...
STATUS="${P}/stage160a_dependency_status.txt"
OUTPUT="${ROOT}/stage160a_invariant_feature_oof_selector"
...
fail_status() {
  local rc=$?
  printf 'stage160a_failed rc=%s at=%s line=%s\n' \
...
"${PYTHON}" -m py_compile \
  ...
  stage160_invariant_feature_selector.py \
  test_stage160_invariant_feature_selector.py
"${PYTHON}" -m unittest -v test_stage160_invariant_feature_selector.py

printf 'stage160a_training_invariant_feature_oof %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" stage160_invariant_feature_selector.py train \
  "${RAW_TRAIN}" "${SOURCE_TRAIN}" "${COMPACT_TRAIN}" \
  "${STAGE31_LOCK}" "${STAGE33_LOCK}" "${STAGE142_LOCK}" \
  "${OUTPUT}" 2>&1 | tee "${P}/stage160a_invariant_feature_oof_train.log"

printf 'stage160a_complete %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
chmod 0444 "${STATUS}"
trap - ERR
