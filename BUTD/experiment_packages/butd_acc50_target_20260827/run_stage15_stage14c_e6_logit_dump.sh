...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage15_stage14c_e6_logit_dump"
CKPT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage14c_rescue_gate_stable/scanrefer_spacy/1787858656/ckpt_best_primary.pth"
PATCHED="${P}/grounding_evaluator_adapter_dump.py"
LIVE_BACKUP="${P}/state/stage15_stage14c_e6_logit_dump_20260828"

[ ! -e "${OUT}" ] || { echo "refusing to overwrite ${OUT}" >&2; exit 215; }
[ -f "${CKPT}" ] && [ -f "${PATCHED}" ]
assert_gpu_idle
assert_storage
...
NMV2_DETECTOR_TOPK_COMPACT_DUMP_PATH="${OUT}/stage14c_e6_logits.pt" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}"

restore
trap - EXIT INT TERM
"${PYTHON}" - "${OUT}/stage14c_e6_logits.pt" <<'PY'
import sys
import torch
