...
STATUS="${P}/stage170a_dependency_status.txt"
OUT="${ROOT}/stage170a_explicit_fix_break_risk_stage154_stage165"

fail_status() {
  local rc=$?
  printf 'stage170a_failed rc=%s at=%s line=%s\n' \
...
grep -q '^stage169a_complete ' "${P}/stage169a_dependency_status.txt"
grep -q '^stage169b_not_authorized_internal_gate_failed ' \
  "${P}/stage169b_dependency_status.txt"
test ! -e "${OUT}"
printf 'stage170a_testing_explicit_fix_break_risk %s\n' \
...
printf 'stage170a_training_explicit_fix_break_risk %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
cd "${R}"
"${PYTHON}" "${P}/stage170_explicit_fix_break_risk.py" train \
  ...
  "${OUT}" \
  2>&1 | tee "${P}/stage170a_explicit_fix_break_risk_train.log"
printf 'stage170a_complete %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
chmod 0444 "${STATUS}"
trap - ERR
