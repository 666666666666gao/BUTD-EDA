...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage12_textcid_reranker"
...
[ ! -e "${OUT}" ] || { echo "refusing to overwrite ${OUT}" >&2; exit 222; }
mkdir -p "${OUT}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" 2>&1 | tee "${P}/stage12_train.log"
