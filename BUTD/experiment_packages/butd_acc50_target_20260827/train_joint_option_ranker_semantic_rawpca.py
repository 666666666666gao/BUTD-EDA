...
PROJECTED_QUERY_KEY = 'adapter_last_proj_query_f16'
PROJECTED_QUERY_DIM = 64
RAW_QUERY_PCA_KEY = 'adapter_last_query_pca64_f16'
RAW_QUERY_PCA_DIM = 64
...
def decode_candidate_f16(row, key, expected_rows, expected_dim):
    """Decode one candidate-aligned FP16 blob with strict shape checks."""
    shape_key = key + '_shape'
    assert key in row and shape_key in row, (key, shape_key)
    shape = tuple(int(value) for value in row[shape_key])
    assert shape == (int(expected_rows), int(expected_dim)), (
        key, shape, expected_rows, expected_dim
    )
    packed = row[key]
    if isinstance(packed, torch.Tensor):
        values = packed.detach().cpu().numpy().astype(np.float32, copy=False)
    elif isinstance(packed, np.ndarray):
        values = packed.astype(np.float32, copy=False)
    else:
        assert isinstance(packed, (bytes, bytearray, memoryview)), type(packed)
        values = np.frombuffer(packed, dtype=np.float16).reshape(shape).astype(
            np.float32
        )
    assert values.shape == shape, (values.shape, shape)
...
def feature_names():
    ...
    names += ['pred_log_size_x', 'pred_log_size_y', 'pred_log_size_z',
              'pred_log_volume']
    names += [
        'proj_query_{:02d}'.format(index)
        for index in range(PROJECTED_QUERY_DIM)
    ]
    names += [
        'proj_query_minus_baseline_{:02d}'.format(index)
        for index in range(PROJECTED_QUERY_DIM)
    ]
    names += [
        'proj_query_minus_mean_{:02d}'.format(index)
        for index in range(PROJECTED_QUERY_DIM)
    ]
    names += [
        'proj_query_norm', 'proj_query_minus_baseline_l2',
        'proj_query_minus_mean_l2', 'proj_query_cos_baseline',
    ]
    names += [
        'raw_pca_query_{:02d}'.format(index)
        for index in range(RAW_QUERY_PCA_DIM)
    ]
    names += [
        'raw_pca_query_minus_baseline_{:02d}'.format(index)
        for index in range(RAW_QUERY_PCA_DIM)
    ]
    names += [
        'raw_pca_query_minus_mean_{:02d}'.format(index)
        for index in range(RAW_QUERY_PCA_DIM)
    ]
    names += [
        'raw_pca_query_norm', 'raw_pca_query_minus_baseline_l2',
        'raw_pca_query_minus_mean_l2', 'raw_pca_query_cos_baseline',
    ]
...
def row_options(row, max_candidates=8):
    ...
    projected_queries = decode_candidate_f16(
        row, PROJECTED_QUERY_KEY, len(queries), PROJECTED_QUERY_DIM
    )
    raw_pca_queries = decode_candidate_f16(
        row, RAW_QUERY_PCA_KEY, len(queries), RAW_QUERY_PCA_DIM
    )
    ...
    baseline_projected_query = projected_queries[baseline_position]
    mean_projected_query = projected_queries.mean(axis=0)
    baseline_projected_query_norm = float(
        np.linalg.norm(baseline_projected_query)
    )
    baseline_raw_pca_query = raw_pca_queries[baseline_position]
    mean_raw_pca_query = raw_pca_queries.mean(axis=0)
    baseline_raw_pca_query_norm = float(
        np.linalg.norm(baseline_raw_pca_query)
    )
    ...
    for position in positions:
        ...
        projected_query = projected_queries[position]
        projected_minus_baseline = (
            projected_query - baseline_projected_query
        )
        projected_minus_mean = projected_query - mean_projected_query
        projected_query_norm = float(np.linalg.norm(projected_query))
        projected_cos_baseline = float(
            np.dot(projected_query, baseline_projected_query)
            / max(
                projected_query_norm * baseline_projected_query_norm,
                1e-8,
            )
        )
        candidate_features += list(projected_query)
        candidate_features += list(projected_minus_baseline)
        candidate_features += list(projected_minus_mean)
        candidate_features += [
            projected_query_norm,
            float(np.linalg.norm(projected_minus_baseline)),
            float(np.linalg.norm(projected_minus_mean)),
            projected_cos_baseline,
        ]
        raw_pca_query = raw_pca_queries[position]
        raw_pca_minus_baseline = raw_pca_query - baseline_raw_pca_query
        raw_pca_minus_mean = raw_pca_query - mean_raw_pca_query
        raw_pca_query_norm = float(np.linalg.norm(raw_pca_query))
        raw_pca_cos_baseline = float(
            np.dot(raw_pca_query, baseline_raw_pca_query)
            / max(
                raw_pca_query_norm * baseline_raw_pca_query_norm,
                1e-8,
            )
        )
        candidate_features += list(raw_pca_query)
        candidate_features += list(raw_pca_minus_baseline)
        candidate_features += list(raw_pca_minus_mean)
        candidate_features += [
            raw_pca_query_norm,
            float(np.linalg.norm(raw_pca_minus_baseline)),
            float(np.linalg.norm(raw_pca_minus_mean)),
            raw_pca_cos_baseline,
        ]
...
def self_test():
    ...
    projected_queries = np.linspace(
        -1.0, 1.0, 3 * PROJECTED_QUERY_DIM, dtype=np.float32
    ).reshape(3, PROJECTED_QUERY_DIM).astype(np.float16)
    row[PROJECTED_QUERY_KEY] = projected_queries.tobytes(order='C')
    row[PROJECTED_QUERY_KEY + '_shape'] = list(projected_queries.shape)
    raw_pca_queries = np.linspace(
        1.0, -1.0, 3 * RAW_QUERY_PCA_DIM, dtype=np.float32
    ).reshape(3, RAW_QUERY_PCA_DIM).astype(np.float16)
    row[RAW_QUERY_PCA_KEY] = raw_pca_queries.tobytes(order='C')
    row[RAW_QUERY_PCA_KEY + '_shape'] = list(raw_pca_queries.shape)
