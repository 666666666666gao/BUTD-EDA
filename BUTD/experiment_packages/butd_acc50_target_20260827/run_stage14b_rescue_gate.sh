...
OUT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage14b_rescue_gate"
...
CMD+=(--log_dir "${OUT}" --max_epoch "$((E+3))" --val_freq 1 --save_freq 1000
 ...
 --detector_policy_adapter_lr 0.0005 --detector_policy_adapter_loss_weight 1.0
...
assert_gpu_idle
assert_storage
[ ! -e "${OUT}" ] || { echo "refusing to overwrite ${OUT}" >&2; exit 225; }
mkdir -p "${OUT}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" 2>&1 | tee "${P}/stage14b_train.log"
