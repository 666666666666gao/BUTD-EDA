...
OUTDIR="${ROOT}/stage144_stage135c_same_checkpoint_fixbreak_gate"
ARTIFACTS="${OUTDIR}/artifacts"
POLICY="${ARTIFACTS}/locked_fixbreak_gate.json"
RESULT="${OUTDIR}/stage144_on_stage135c_eval.json"
LOG="${OUTDIR}/stage144.log"
...
EXPECTED_STAGE144_SHA=a047055778bec52058a5271d9c1bc1279ef961c64bc48fc97990e9494834ad76
...
check_sha "${P}/stage144_same_checkpoint_fixbreak_gate.py" "${EXPECTED_STAGE144_SHA}"
...
{
  printf 'stage=144_stage135c_same_checkpoint_fixbreak_gate\n'
...
"${PY}" stage144_same_checkpoint_fixbreak_gate.py train \
  "${TRAIN_DUMP}" "${STAGE31_LOCK}" "${STAGE33_LOCK}" \
  "${STAGE142_LOCK}" "${ARTIFACTS}" 2>&1 | tee "${LOG}"
test -s "${POLICY}"
sha256sum "${POLICY}" > "${OUTDIR}/locked_fixbreak_gate.json.sha256"
...
if [ "${AUTHORIZED}" = true ]; then
  "${PY}" stage144_same_checkpoint_fixbreak_gate.py evaluate \
    "${VAL_DUMP}" "${POLICY}" "${RESULT}" 2>&1 | tee -a "${LOG}"
  test -s "${RESULT}"
  sha256sum "${RESULT}" > "${OUTDIR}/stage144_on_stage135c_eval.json.sha256"
else
