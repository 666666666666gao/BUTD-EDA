...
def feature_names():
    ...
    names += ['match_rank', 'match_score', 'match_score_gap_to_top1']
    ...
    for position in positions:
        ...
        for match_power in MATCH_POWERS:
            if len(det_boxes):
                ...
                match_order = np.argsort(
                    -match_score, kind='stable'
                )[:MATCH_TOPK]
            else:
                match_score = np.zeros(1, dtype=np.float32)
                match_order = np.asarray([-1], dtype=np.int64)
            top_match_score = float(match_score[match_order[0]])
            for match_rank, match_index in enumerate(match_order):
                if match_index >= 0:
                    matched = det_boxes[match_index]
                    matched_support = float(support[match_index])
                    matched_conf = float(det_conf[match_index])
                    selected_match_score = float(match_score[match_index])
                else:
                    matched = pred.copy()
                    matched_support = 0.0
                    matched_conf = 0.0
                ...
                pair_features += [
                    float(match_power),
                    float(match_rank) / max(MATCH_TOPK - 1, 1),
                    selected_match_score,
                    top_match_score - selected_match_score,
                ]
                for action_index, alpha in enumerate(ACTIONS):
                    ...
                    action_features += [
                        float(np.log(np.prod(option_size) + 1e-8))
                    ]
                    vector = (
                    ...
                    ious.append(float(
                        aligned_iou(option[None], gt_box[None])[0]
                    ))
...
def evaluate(args):
    ...
    assert int(lock['match_topk']) == MATCH_TOPK
...
def binary50_train(args):
    ...
    receipt = {
        'protocol': 'scene_hash_train_only_binary50_top2_match_config_selection',
        'train_dump': os.path.abspath(args.train_dump),
        'train_dump_sha256': sha256(args.train_dump),
        'script_sha256': sha256(os.path.abspath(__file__)),
        'model_path': os.path.abspath(model_path),
...
def self_test():
    row = {
        ...
        'detected_target_confidence': [0.9, 0.8],
        'text_target_cid': 1,
    ...
    assert x.shape == (
        3 * len(MATCH_POWERS) * MATCH_TOPK * len(ACTIONS),
        len(FEATURE_NAMES),
    )
