...
BACKUP="${P}/state/stage98_stage95_e11_geometry_dump_20260830"
...
RESULT="${ROOT}/stage99_stage29_on_stage95_locked_val_eval.json"
STATUS="${P}/stage98_99_chain_status.txt"
...
EXPECTED_PATCHED_SHA="ec30597068e61be02cec73485beec2d111b7b6454336f49cced05bc953c5ac55"
...
mkdir -p "${BACKUP}" "${OUT}"
cp -p "${LIVE}" "${BACKUP}/grounding_evaluator.pre_stage98.py"
chmod 0444 "${BACKUP}/grounding_evaluator.pre_stage98.py"
restore() {
  install -m 0644 "${BACKUP}/grounding_evaluator.pre_stage98.py" "${LIVE}"
}
...
printf 'stage98_dumping %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
NMV2_DETECTOR_TOPK_COMPACT_DUMP_PATH="${DUMP}" \
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
  2>&1 | tee "${P}/stage98_stage95_geometry_dump.log"
...
print('STAGE98_DUMP_PARITY_PASS rows={} acc025={:.12f} acc050={:.12f} bytes={}'.format(
    len(rows), actual[0], actual[1], os.path.getsize(dump)))
PY

printf 'stage99_evaluating %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" "${RANKER_SCRIPT}" evaluate \
  "${DUMP}" "${MODEL}" "${LOCK}" "${RESULT}" \
  2>&1 | tee "${P}/stage99_stage29_on_stage95_locked_val_eval.log"
test -s "${RESULT}"
...
path = os.path.join(out, 'stage98_99_receipt.json')
with open(path, 'w', encoding='utf-8') as f:
    json.dump(receipt, f, indent=2, sort_keys=True)
    f.write('\n')
print(json.dumps(receipt, indent=2, sort_keys=True))
PY
chmod 0444 "${OUT}/stage98_99_receipt.json" "${OUT}/launch_manifest.txt"
printf 'stage99_complete %s\n' "$(date --iso-8601=seconds)" > "${STATUS}"
