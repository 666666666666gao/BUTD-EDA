...
OUT="${ROOT}/stage135c_stage29_option_last_box_noaug_jointmask"
STATUS="${P}/stage135c_option_last_box_status.txt"
...
printf 'stage135c_training %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" "${CMD[@]}" \
  2>&1 | tee "$P/stage135c_option_last_box_train.log"
printf 'stage135c_complete %s\n' "$(date --iso-8601=seconds)" > "$STATUS"
