...
TRAIN_RUN="${1:?usage: verify_stage2_reload.sh TRAIN_RUN_DIR}"
...
VERIFY_ROOT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage2_reload_verify"
...
[ -f "${CHECKPOINT}" ] || {
  echo "ERROR: stage2 checkpoint missing: ${CHECKPOINT}" >&2
  exit 100
}
[ ! -e "${VERIFY_ROOT}" ] || {
