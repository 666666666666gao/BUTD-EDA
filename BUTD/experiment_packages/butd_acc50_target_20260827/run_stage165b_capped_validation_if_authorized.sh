...
STATUS="${P}/stage165b_dependency_status.txt"
...
RAW_RESULT="${ROOT}/stage165b_capped_raw_validation_result.json"
RESULT="${ROOT}/stage165b_capped_validation_result.json"

fail_status() {
  local rc=$?
  printf 'stage165b_failed rc=%s at=%s line=%s\n' \
...
grep -q '^stage165a_complete ' "${P}/stage165a_dependency_status.txt"
AUTHORIZED=$("${PYTHON}" - "${AUTH}" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]))
assert x['stage']=='165_stage164_nested_blend_train_dev_90pct_change_cap'
...
if test "${AUTHORIZED}" != "1"; then
  printf 'stage165b_not_authorized_internal_gate_failed %s\n' \
...
test ! -e "${RAW_RESULT}"
test ! -e "${RESULT}"
printf 'stage165b_locked_capped_policy_evaluating %s\n' \
  "$(date --iso-8601=seconds)" > "${STATUS}"
"${PYTHON}" "${P}/stage140_train_eval_nested_blend.py" evaluate \
  "${RAW_VAL}" "${POLICY}" "${RAW_RESULT}" \
  2>&1 | tee "${P}/stage165b_capped_validation.log"
"${PYTHON}" "${P}/stage165_capped_nested_gate.py" wrap-validation \
  "${AUTH}" "${RAW_RESULT}" "${RESULT}" \
  2>&1 | tee -a "${P}/stage165b_capped_validation.log"
...
printf 'stage165b_complete_%s %s\n' "${MET}" \
  "$(date --iso-8601=seconds)" > "${STATUS}"
