...
CHECKPOINT="${1:?usage: verify_stage5_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
QUALITY_WEIGHT="${2:?usage: verify_stage5_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
CALIBRATION_JSON="${3:?usage: verify_stage5_reload.sh CHECKPOINT QUALITY_WEIGHT CALIBRATION_JSON}"
...
VERIFY_ROOT="/root/autodl-tmp/logs/butd_acc50_target_20260827/stage5_reload_verify"
...
[ -f "${CALIBRATION_JSON}" ]
[ ! -e "${VERIFY_ROOT}" ] || {
  echo "ERROR: refusing to overwrite Stage-5 verification root ${VERIFY_ROOT}" >&2
  exit 180
}
