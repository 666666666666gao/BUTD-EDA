...
OUT="${ROOT}/stage151c_stage150_e13_true_all_query_oracle_dump"
DUMP="${OUT}/stage150_e13_true_all_query_oracle_geometry.pt"
FIXBREAK="${OUT}/stage151c_stage150_fix_break_report.json"
TIER_REPORT="${OUT}/stage151c_true_all_query_oracle_report.json"
STATUS="${P}/stage151c_true_all_query_oracle_status.txt"
LIVE="src/grounding_evaluator.py"
PATCHED="${P}/grounding_evaluator_stage151c_true_all_query_oracle.py"
COMPARE="${P}/compare_stage149b_calibrated_fix_break.py"
ANALYZE="${P}/analyze_stage151c_true_all_query_oracle.py"
STATE="${P}/state/stage151c_stage150_true_all_query_oracle_20260831"
...
EXPECTED_PATCHED_SHA="de1a56f2125de1fa265411d18ebf500ce32862ce2e73d1645de39c29aee10a11"
...
fail_status() {
  local rc=$?
  printf 'stage151c_failed rc=%s at=%s line=%s\n' \
...
assert_storage
mkdir -p "${OUT}" "${STATE}"
cp -p "${LIVE}" "${STATE}/grounding_evaluator.pre_stage151c.py"
chmod 0444 "${STATE}/grounding_evaluator.pre_stage151c.py"
restore() {
  install -m 0644 "${STATE}/grounding_evaluator.pre_stage151c.py" "${LIVE}"
}
...
{
  printf 'stage=151c_stage150_true_all_query_oracle\n'
  printf 'started_at=%s\ncheckpoint=%s\ncheckpoint_sha256=%s\n' \
...
print('STAGE151C_STAGE150_TRUE_ALL_QUERY_ORACLE_PARITY_PASS', hits)
PY

"${PYTHON}" "${COMPARE}" "${BASELINE_DUMP}" "${DUMP}" "${FIXBREAK}" \
  > "${P}/stage151c_stage150_fix_break_compare.log"
"${PYTHON}" "${ANALYZE}" "${BASELINE_DUMP}" "${DUMP}" "${TIER_REPORT}" \
  > "${P}/stage151c_true_all_query_oracle_analysis.log"
...
print('STAGE151C_TRUE_ALL_QUERY_REPORT_PARITY_PASS')
PY
