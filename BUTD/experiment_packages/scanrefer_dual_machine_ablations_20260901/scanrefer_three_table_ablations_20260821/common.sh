...
LOG_ROOT="${ABLATION_LOG_ROOT:-/root/autodl-tmp/logs/butd_scanrefer_three_table_ablations_20260821/seed0}"
...
MASTER_PORT="${MASTER_PORT:-$((32000 + RANDOM % 500))}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
MIN_FREE_KIB="${ABLATION_MIN_FREE_KIB:-1600000}"
...
if pgrep -af 'train_dist_mod.py' >/dev/null 2>&1; then
  echo "ERROR: another training process is active; refusing parallel training." >&2
  pgrep -af 'train_dist_mod.py' >&2 || true
  exit 41
fi

mkdir -p "$(dirname "${LOG_ROOT}")"
FREE_KIB="$(df -Pk "$(dirname "${LOG_ROOT}")" | awk 'NR==2 {print $4}')"
if [ "${FREE_KIB}" -lt "${MIN_FREE_KIB}" ]; then
  echo "ERROR: only ${FREE_KIB} KiB free; storage gate requires ${MIN_FREE_KIB} KiB." >&2
  exit 42
fi
...
if [ "${ALLOW_BUSY_GPU:-0}" != "1" ] && [ "${GPU_USED_MIB}" -ge 500 ]; then
  echo "ERROR: GPU ${GPU_INDEX} is busy (${GPU_USED_MIB} MiB)." >&2
  exit 43
fi
