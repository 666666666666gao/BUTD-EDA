...
def get_scheduler(optimizer, n_iter_per_epoch, args):
    if "cosine" in args.lr_scheduler:
        ...
        # Without warm-up, an epoch milestone means decay immediately after
        # that epoch's final optimizer step. The previous expression added one
        # epoch when warmup_epoch == -1, silently shifting every requested
        # milestone later.
        if args.warmup_epoch > 0:
            milestone_steps = [
                (m - args.warmup_epoch) * n_iter_per_epoch
                for m in args.lr_decay_epochs
            ]
        else:
            milestone_steps = [
                m * n_iter_per_epoch for m in args.lr_decay_epochs
            ]
