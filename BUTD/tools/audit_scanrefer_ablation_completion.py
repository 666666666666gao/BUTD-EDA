...
def audit_job(job_id, expected, summary):
    ...
    require(5 <= best_epoch <= 65 and best_epoch % 5 == 0, "invalid best epoch for {}".format(job_id))
    ...
    require(len(validations) == 13, "{} has {} validation logs, expected 13".format(job_id, len(validations)))
    epochs = sorted(epoch for epoch, _ in validations)
    require(epochs == list(range(5, 66, 5)), "{} validation epochs are incomplete".format(job_id))
