...
def audit_job(job_id, expected, summary):
    ...
    require(len(validations) == 13, "{} expected 13 validations".format(job_id))
    require(sorted(epoch for epoch, _ in validations) == list(range(5, 66, 5)), "validation epochs incomplete")
...
def main():
    ...
    lines = [
        ...
        "All three extension rows independently retrained for exactly 65 epochs with learning-rate decays after epochs 55 and 60. Each retains exactly one strict-best official Overall Acc@0.25 checkpoint and passed final best-checkpoint reload evaluation.", "",
